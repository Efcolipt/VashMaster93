...
require 'db.php';
$person = mysqli_query($link,"SELECT * FROM ".$datatablePerson.""); 
?>
					...
					<div class="error__message"><p></p></div>
						...
						<? while($rows = mysqli_fetch_assoc($person)): ?>
								<p data-id="<?=$rows["id"];?>">
									...
									<span>Логин: <?=$rows['login']; ?></span>
									<span>Имя: <?=$rows['name']; ?></span>
									<span>Админ или модератор: <?php if ($rows['isAdmin'] == true) {
										echo "Админ";
									}else{echo "Модератор";} ?></span>
									<span>Дата: <?=$rows['date_register']; ?></span>
									<span class="delete__app" data-name-page="users"><button class="btn__default">Удалить</button></span>
								</p>
						<? endwhile; ?>
