...
require 'db.php';
$portfolio = mysqli_query($link,"SELECT * FROM ".$datatablePortfolio.""); 
?>
					...
					<div class="error__message"><p></p></div>
						...
						<? while($rows = mysqli_fetch_assoc($portfolio)): ?>
								<p data-id="<?=$rows["id"];?>">
									...
									<span>Фото: <img src="../image/portfolio/<?=$rows['path']; ?>" alt="<?=$rows['name']; ?>"></span>
									<span>Дата: <?=$rows['date_add']; ?></span>
									<span class="delete__app" data-name-page="portfolio"><button class="btn__default">Удалить</button></span>
								</p>
						<? endwhile; ?>
